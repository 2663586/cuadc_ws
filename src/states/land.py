...
from config import LAND_DESCEND_RATE_MPS


class PrecisionLandState(BaseState):
    """普通降落：飞回原点上方，逐步下降，触发自动降落。"""
    ...
    async def enter(self, interface):
        ...
        print(f"[降落] 返回原点上方 {alt:.1f} 米，准备降落")
    ...
    async def execute(self, interface):
        ...
        # 逐周期下降
        target_alt = max(alt - LAND_DESCEND_RATE_MPS, 0.3)
        sp = interface.field_to_ned(0.0, 0.0, target_alt)
        interface.update_setpoint(sp)
        return False, None
