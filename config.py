...
# Transit 状态（增量位置斜坡方案）
#
# 每周期斜坡推进 step = speed / FSM_LOOP_HZ 米。
# PX4 内部 Position Controller 负责实际飞行动力学。
# 相关 PX4 参数（可通过 MAVSDK param 接口设）：
#   MPC_XY_VEL_MAX — 最大水平速度（兜底限速）
#   MPC_ACC_HOR    — 最大水平加速度
#   MPC_XY_P       — 位置环 P 增益，决定稳态追踪滞后量
# ---------------------------------------------------------------------------
